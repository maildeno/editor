<template>
  <Transition name="panel-slide">
    ...
    -->
    <div v-if="isOpen" class="w-65 sticky top-16 h-[calc(100vh-5.25rem)] z-80">
      <div
 ...
 ───────────────────────────────────────────────────────────────────── -->
        <div class="flex items-center justify-between px-3.5 py-3 shrink-0">
          ...
          <div class="relative group/close">
            ...
            <div
              class="pointer-events-none absolute right-0 top-full mt-1.5 px-2 py-1 bg-(--md-tooltip-bg) text-(--md-tooltip-text) text-[10px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/close:opacity-100 -translate-y-0.5 group-hover/close:translate-y-0 transition-all duration-150 z-50 after:content-[''] after:absolute after:bottom-full after:right-2 after:border-4 after:border-transparent after:border-b-(--md-tooltip-bg)"
            >
              Close panel
            </div>
          </div>
        ...
        <div class="flex-1 min-h-0 overflow-y-auto">
          ...
          <div v-if="current.isFetching.value" class="p-2 flex flex-col gap-2">
            ...
            >
              <div
                ...
                :style="{ height: 80 + n * 20 + 'px' }"
              />
              <div class="flex items-center gap-2 px-2.5 py-2">
                <div
                  class="h-2 flex-1 rounded-full bg-(--md-surface-muted) animate-pulse"
                />
                <div
                  class="h-2 w-7 rounded-full bg-(--md-surface-muted) animate-pulse"
                />
              </div>
            </div>
          </div>
          ...
          >
            <div
              class="w-10 h-10 rounded-xl bg-(--md-surface-hover) border border-(--md-border)/80 flex items-center justify-center"
            >
              <svg
          ...
          <div v-else class="p-2 flex flex-col gap-1.5">
            <TransitionGroup name="row-list">
              <div
                ...
                :key="entry.id"
                draggable="true"
                class="group relative rounded-lg border bg-(--md-surface) overflow-hidden transition-all duration-150 select-none cursor-grab active:cursor-grabbing"
                :class="[
                  draggingId === entry.id
                    ? 'ring-2 ring-(--md-row-selection) ring-offset-1 opacity-60 border-(--md-row-selection)'
                    : 'border-(--md-border)/80 hover:border-(--md-border-strong) hover:shadow-[0_2px_8px_rgba(0,0,0,0.07)]',
                ]"
              ...
              >
                ...
                <div
                  :ref="
                    (el) => observeContainerEl(el as Element | null, entry.id)
                  "
                ...
                >
                  <div
                    :ref="
                      (el) => observePreviewEl(el as Element | null, entry.id)
                    "
                  ...
                  <div
                    class="absolute inset-0 bg-(--md-text)/0 group-hover:bg-(--md-text)/[0.03] transition-colors duration-150 flex items-center justify-center"
                  >
                    <div
                      class="opacity-0 group-hover:opacity-100 transition-opacity duration-150 flex items-center gap-1.5 bg-(--md-surface)/90 backdrop-blur-sm border border-(--md-border) rounded-full px-2.5 py-1 shadow-sm"
                    >
                      <svg
                      ...
                      <span
                        class="text-[10px] font-medium text-(--md-text-muted)"
                ...
                >
                  <!-- Drag handle -->
                  <div
                    class="shrink-0 text-(--md-text-subtle) group-hover:text-(--md-text-muted) transition-colors cursor-grab active:cursor-grabbing"
                  >
                    <svg
                      class="w-3 h-3"
                      viewBox="0 0 16 16"
                  ...
                  <div v-if="canDeleteEntry" class="relative group/del">
                    ...
                    <div
                      class="pointer-events-none absolute bottom-full right-0 mb-1.5 px-2 py-1 bg-(--md-tooltip-bg) text-(--md-tooltip-text) text-[10px] font-medium rounded-md whitespace-nowrap opacity-0 group-hover/del:opacity-100 translate-y-0.5 group-hover/del:translate-y-0 transition-all duration-150 z-50 after:content-[''] after:absolute after:top-full after:right-2 after:border-4 after:border-transparent after:border-t-(--md-tooltip-bg)"
                    >
                      Remove
                    </div>
                  </div>
                ...
                <div v-else class="flex items-center gap-1.5 px-2 py-1.5">
                  <span
                    class="flex-1 text-[11px] font-medium text-(--md-text-muted) truncate"
                  >
                    {{ entry.name }}
                  </span>
                  <span
