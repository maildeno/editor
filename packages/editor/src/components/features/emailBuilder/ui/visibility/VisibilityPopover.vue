      ...
      <template v-if="visibility.rules?.length">
        ...
        >
          ...
          <div class="flex items-center gap-1.5 flex-wrap">
            <span
              class="bg-[#0f6e75] text-gray-100 ring-1 ring-[#0f6e75] rounded-md px-1.5 py-0.5 text-[10px] font-mono font-semibold"
            >
              {{ rule.tag || "?" }}
            </span>
            <span
...
const totalRuleCount = computed(() => {
  ...
  const groupCount = (props.visibility.groups ?? []).reduce(
