<template>
  ...
  <template v-if="bare">
    <div class="flex items-center gap-2">
      <button
        ref="triggerRef"
        class="w-5.25 h-5.25 rounded-full border border-black/10 shrink-0 cursor-pointer transition-all hover:scale-110 hover:shadow-sm focus:outline-none"
      ...
      <input
        v-if="showInput"
        type="text"
        class="w-full h-8 py-2 px-2 text-[13px] text-gray-700 border border-gray-300/80 rounded-md bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)] focus:outline-none transition-all"
        :value="modelValue"
  ...
  <div v-else class="space-y-1.5">
    ...
    <div v-if="allowTransparent" class="flex items-center gap-2">
      <button
        ...
        :class="isTransparent ? 'bg-green-400' : 'bg-gray-200'"
        role="switch"
        :aria-checked="isTransparent"
        @click="onTransparentToggle(!isTransparent)"
      >
        <span
          class="absolute top-0.5 h-3.25 w-3.5 rounded-full bg-white shadow-sm transition-all duration-200"
