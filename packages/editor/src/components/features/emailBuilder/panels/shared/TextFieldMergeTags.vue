<template>
  <div class="border border-gray-200/80 rounded-lg p-3 bg-white space-y-2.5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
    ...
    <div class="flex flex-wrap gap-1.5">
      ...
      >
        <button
          type="button"
          @click="prefill(tag)"
          class="relative px-2 py-1 text-xs bg-gray-50 text-gray-600 border border-gray-200 rounded-md hover:border-green-200 hover:bg-green-50 hover:text-green-700 transition-all focus:outline-none"
        >
          <span>&#123;&#123; {{ tag }} &#125;&#125;</span>
    ...
    <div class="space-y-1.5">
      <div class="flex items-center gap-1.5">
        ...
        <div class="relative flex-1">
          ...
          <input
            ...
            class="w-full h-[30px] pl-5 pr-2 text-[13px] border border-gray-200/80 rounded-md bg-white shadow-[0_1px_2px_rgba(0,0,0,0.04)] focus:outline-none transition-colors"
        ...
        <button
          ...
          @click="insertCustom"
          class="h-[30px] px-3 text-[13px] bg-gray-900 text-white rounded-md hover:bg-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors whitespace-nowrap focus:outline-none"
        >
          Insert
        </button>
      </div>
      ...
      <p v-if="customTag.trim()" class="text-[10px] text-gray-500 break-all font-mono">
        {{ tokenPreview }}
      </p>
    </div>
