<template>
  <div class="border border-(--md-border) rounded-lg bg-(--md-surface) overflow-hidden">
    ...
    <div class="p-3 space-y-3">
      <div>
        <label
          class="block text-[10px] text-(--md-text-subtle) mb-1.5 uppercase tracking-wide"
        >
          Target ESP Syntax
        </label>
        <div class="relative">
          <select
            ...
            class="w-full text-sm px-2.5 py-1.5 pr-7 outline-1 outline-(--md-border) rounded-md shadow-xs focus:outline-none focus:ring-[1px] focus:ring-(--md-selection) bg-(--md-surface) text-(--md-text) appearance-none cursor-pointer"
          >
            <optgroup
          ...
          <Icon
            name="chevron-down"
            class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-(--md-text-subtle)"
        ...
        <div class="mt-1.5 flex items-center justify-between">
          ...
          <a
            ...
            class="text-[10px] text-(--md-info) hover:text-(--md-info-fg) flex items-center gap-0.5 ml-2 shrink-0"
          >
            docs
      ...
      <div
        ...
        class="flex items-start gap-2 bg-(--md-danger-bg) border border-(--md-danger-border) rounded-md px-3 py-2.5"
      >
        ...
        <div class="text-[10px] text-(--md-danger-fg) leading-relaxed space-y-0.5">
          ...
          <ul class="list-none space-y-0.5 mt-1">
            <li
              ...
              :key="item.key"
              class="font-mono bg-(--md-danger-border) rounded px-1.5 py-0.5 inline-flex items-center gap-1 mr-1"
            >
              <span class="text-(--md-danger)">✕</span> {{ item.label }}
            </li>
          </ul>
          <p class="mt-1 text-(--md-danger)/80">
        ...
        <div class="text-[10px] text-(--md-warning-fg) leading-relaxed space-y-0.5">
          ...
          <ul class="list-none space-y-0.5 mt-1">
            <li
              ...
              :key="item.key"
              class="font-mono bg-(--md-warning-border) rounded px-1.5 py-0.5 inline-flex items-center gap-1 mr-1"
            >
              <span class="text-(--md-warning)">⚠</span> {{ item.label }}
            </li>
          </ul>
          <p class="mt-1 text-(--md-warning-fg)/80">
            {{ degradedOperators[0]?.hint }}
          </p>
          <p
            v-if="degradedOperators.some((o) => o.key === 'date')"
            class="mt-1.5 text-(--md-warning-fg) font-medium flex items-start gap-1"
          >
      ...
      <div
        ...
        class="flex items-start gap-2 bg-(--md-info-bg) border border-(--md-info-border) rounded-md px-3 py-2.5"
      >
        <span class="text-(--md-info) mt-0.5 shrink-0" style="font-size: 11px"
          >ℹ</span
        >
        ...
        <table class="text-[10px] border-collapse min-w-full">
          ...
          <tbody>
            <tr
              ...
              :key="row.syntax"
              :class="[
                row.syntax === espConfig.syntax ? 'bg-(--md-selection-bg)' : '',
                'hover:bg-(--md-surface-hover)/50 transition-colors',
              ]"
            >
              <td
