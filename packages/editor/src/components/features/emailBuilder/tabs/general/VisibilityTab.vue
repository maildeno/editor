<template>
  <div
    class="border border-(--md-border) rounded-lg bg-(--md-surface) overflow-hidden flex flex-col"
  >
    <!-- Header -->
    <div
      class="flex items-center justify-between px-3 py-2 bg-(--md-surface-hover) border-b border-(--md-border) shrink-0"
    >
      ...
      <button
        ...
        class="text-(--md-text-subtle) hover:text-(--md-text-muted) text-xs flex items-center gap-1"
      >
    ...
    <div class="p-3 space-y-2 shrink-0">
      ...
      <div v-if="activeCount === 0" class="text-center py-3">
        <span class="text-[11px] text-(--md-text-subtle)"
          >No active context tags — add one to preview visibility rules</span
        >
      </div>
    </div>
    ...
    <!-- Conditional Blocks -->
    <div
      v-if="conditionalBlocks.length > 0"
      class="border-t border-(--md-border) flex flex-col min-h-0"
    >
      <button
        @click="showConditionalSummary = !showConditionalSummary"
      ...
      >
        ...
        <svg
          ...
          :class="showConditionalSummary ? 'rotate-180' : ''"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
          viewBox="0 0 24 24"
      ...
      <div
        v-if="showConditionalSummary"
        class="overflow-y-auto divide-y divide-(--md-border)"
        style="max-height: 320px"
      >
        <div
          v-for="block in conditionalBlocks"
          :key="block.id"
          class="p-3 hover:bg-(--md-surface-hover)/40 transition-colors"
        >
          <h4 class="text-[10px] text-(--md-text-muted) font-mono flex-1 mb-1">
            {{ block.label }}
          </h4>
          ...
          <div v-if="block.rules.length > 0" class="space-y-1.5 pl-1">
            <div
              v-for="(rule, ri) in block.rules"
              :key="ri"
              class="flex flex-col gap-0.5"
            >
              <div
                v-if="ri > 0"
                class="text-[8px] font-bold text-(--md-text-subtle) uppercase tracking-widest pl-0.5"
              >
                {{ block.match === "all" ? "AND" : "OR" }}
              </div>
              <div class="flex items-center gap-1.5 flex-wrap">
                ...
                >
                  <Icon
                    :name="isRuleMatched(rule) ? 'check-circle' : 'circle'"
                    style="font-size: 9px"
                  />
          ...
          <div v-if="block.groups?.length" class="mt-2 space-y-2">
            <div
              v-if="block.rules.length > 0"
              class="flex items-center gap-1 pl-1"
            >
              <div class="h-px flex-1 bg-(--md-border)" />
              <span
                class="text-[8px] text-(--md-text-subtle) uppercase font-bold px-1"
              >
                {{ block.match === "all" ? "AND" : "OR" }}
              </span>
              <div class="h-px flex-1 bg-(--md-border)" />
            ...
            >
              <div
                class="text-[8px] font-bold text-(--md-text-subtle) uppercase tracking-widest mb-1.5"
              >
    ...
    <div v-else class="px-3 py-4 text-center border-t border-(--md-border)">
      <Icon
        name="eye-slash"
        class="text-(--md-text-subtle) block mb-1"
        style="font-size: 18px"
      />
      <p class="text-[11px] text-(--md-text-subtle)">
        No visibility rules configured yet
      </p>
    </div>
  </div>
</template>
...
const contextEntries = ref<{ key: string; value: string }[]>(
  Object.keys(visibilityPreviewContext.value).length > 0
    ? Object.entries(visibilityPreviewContext.value).map(([key, value]) => ({
        key,
        value,
      }))
    : [{ key: "", value: "" }],
...
const activeCount = computed(
  () => contextEntries.value.filter((e) => e.key.trim()).length,
);
...
const isRuleMatched = (rule: {
  ...
  operator: string;
  value: string;
}): boolean => evaluateRule(rule as any, visibilityPreviewContext.value);
...
const removeEntry = (i: number) => {
  contextEntries.value.splice(i, 1);
  syncContext();
};
