<template>
  <div class="border border-gray-200 rounded-lg bg-white overflow-hidden flex flex-col">
    <!-- Header -->
    <div class="flex items-center justify-between px-3 py-2 bg-gray-50 border-b border-gray-200 shrink-0">
      <div class="flex items-center gap-2">
        <span class="text-xs font-medium text-gray-600">Preview Context</span>
        <span class="text-[10px] text-gray-400 bg-white px-1.5 py-0.5 rounded-full border border-gray-200">
          {{ activeCount }} active
        </span>
      </div>
      <button @click="addContextEntry" class="text-gray-400 hover:text-gray-600 text-xs flex items-center gap-1">
    ...
    <div class="p-3 space-y-2 shrink-0">
      <div v-if="contextEntries.length > 0" class="grid grid-cols-2 gap-1.5 px-0.5">
        <span class="text-[11px] text-gray-500">Tag</span>
        <span class="text-[11px] text-gray-500">Value</span>
      </div>
      <div v-for="(entry, i) in contextEntries" :key="i" class="flex items-center gap-2 group">
        ...
        <button @click="removeEntry(i)"
          class="opacity-0 group-hover:opacity-100 transition-opacity text-gray-300 hover:text-red-500 shrink-0">
          <Icon name="times" style="font-size: 10px" />
        </button>
      </div>
      <div v-if="activeCount === 0" class="text-center py-3">
        <span class="text-[11px] text-gray-500">No active context tags — add one to preview visibility rules</span>
      </div>
    </div>

    <!-- Conditional Blocks -->
    <div v-if="conditionalBlocks.length > 0" class="border-t border-gray-200 flex flex-col min-h-0">
      <button @click="showConditionalSummary = !showConditionalSummary"
        class="w-full flex items-center justify-between px-3 py-2 bg-gray-50/50 hover:bg-gray-100 transition-colors shrink-0">
        <div class="flex items-center gap-2">
          <span class="text-xs font-medium text-gray-600">Conditional Blocks</span>
          <span class="bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 rounded-full font-medium">
            {{ conditionalBlocks.length }}
          </span>
        </div>
        <svg class="w-3 h-3 text-gray-400 transition-transform" :class="showConditionalSummary ? 'rotate-180' : ''"
          fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      <div v-if="showConditionalSummary" class="overflow-y-auto divide-y divide-gray-100" style="max-height: 320px">
        <div v-for="block in conditionalBlocks" :key="block.id" class="p-3 hover:bg-gray-50/40 transition-colors">
           <h4 class="text-[10px] text-gray-600 font-mono flex-1 mb-1">{{ block.label }}</h4>
          <div class="flex items-center gap-2 mb-2">
            ...
            <span v-if="block.depth > 0"
              class="text-[9px] text-gray-400 bg-gray-100 px-1 py-0.5 rounded font-mono shrink-0">
              depth {{ block.depth }}
            </span>
          ...
          <div v-if="block.rules.length > 0" class="space-y-1.5 pl-1">
            <div v-for="(rule, ri) in block.rules" :key="ri" class="flex flex-col gap-0.5">
              <div v-if="ri > 0" class="text-[8px] font-bold text-gray-400 uppercase tracking-widest pl-0.5">
                {{ block.match === 'all' ? 'AND' : 'OR' }}
              </div>
          ...
          <div v-if="block.groups?.length" class="mt-2 space-y-2">
            <div v-if="block.rules.length > 0" class="flex items-center gap-1 pl-1">
              ...
              <span class="text-[8px] text-gray-400 uppercase font-bold px-1">
                {{ block.match === 'all' ? 'AND' : 'OR' }}
              </span>
              <div class="h-px flex-1 bg-gray-200" />
            </div>
            <div v-for="(group, gi) in block.groups" :key="gi"
              class="border border-gray-200 rounded-md p-2 bg-gray-50/30">
              <div class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">
    ...
    <div v-else class="px-3 py-4 text-center border-t border-gray-100">
      <Icon name="eye-slash" class="text-gray-300 block mb-1" style="font-size: 18px" />
      <p class="text-[11px] text-gray-500">No visibility rules configured yet</p>
    </div>
  </div>
</template>
