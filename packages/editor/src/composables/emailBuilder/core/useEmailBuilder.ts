...
import { normalizeTemplateSnapshot } from "../export/targets/templateSnapshot";
import { useCanSave } from "@/adapters";
...
function createEmailBuilderInstance(
  ...
  const selectedColumn = ref<{ rowId: string; columnId: string } | null>(null);
  ...
  // below now instead.
  const espConfig = ref<{
    syntax: import("../export/transformers/espWrapper").ESPSyntax;
  }>({
    syntax: "handlebars",
  });
  ...
  // Read once at construction: the host's adapter can't change mid-instance.
  ...
  const debouncedSave = () => {
    ...
    // A guest-facing host omits saveTemplate deliberately (see useCanSave).
    // Returning here rather than letting the timer fire means no failed request
    // every few seconds, and no "Save failed" state in a session that was never
    // meant to save. Checked via canSave, not the merged adapter — merging fills
    // saveTemplate from localStorage, so the adapter always has one.
  ...
  const saveTemplate = async (payload: {
    ...
    // Guarded, not assumed: saveTemplate is optional on PartialStorageAdapter,
    // and a guest-facing host omits it deliberately. Without this the autosave
    // timer would keep firing into undefined — a TypeError every few seconds in
    // a session that has no way to save anything.
    if (!canSave.value) {
      // Throws rather than resolving quietly: reaching here means something
      // called save in a session that has no way to save, which is a wiring bug
      // worth surfacing. The autosave scheduler returns early and the Save
      // button is hidden, so nothing in the editor itself gets this far.
      throw new Error(
        "[maildeno-editor] this editor has no saveTemplate — nothing was saved",
      );
    }
  ...
  const loadVersionSnapshot = (
    ...
    // History label for the resulting undo step. Defaults to the version
    // drawer's case, the only caller that existed when this was written.
    // setJson() passes its own so programmatic writes are distinguishable in
    // the undo stack instead of every one claiming to be a version restore.
    historyAction: string = "restore-version",
  ) => {
    if (debounceTimer) {
      clearTimeout(debounceTimer);
      debounceTimer = null;
    ...
    nextTick(() => {
      ...
      saveToHistoryImmediate(historyAction);
    });
  };
  ...
  //
  // The read side (getExported*) has always been public. These are the write
  // side, added so a host can drive the canvas from its own UI — a version
  // drawer restoring a snapshot, an AI assistant applying a generated design
  // — without reaching into rows/canvasStyles directly. Reaching in directly
  // is what the pre-package app did, and it coupled that app to builder
  // internals so tightly that hydration, history and the id registry all had
  // to be replicated by the caller. Everything below routes through the same
  // paths the editor's own UI uses, so those three stay correct for free.

  /**
   * Replaces the entire canvas from a snapshot.
   *
   * Accepts anything normalizeTemplateSnapshot recognizes — notably the exact
   * object getJson() returns, so setJson(getJson()) round-trips, and the
   * { canvas, content: { rows } } shape a version API typically returns.
   *
   * history:
   *   "undoable" (default) — one undo step; the user can Ctrl+Z back to what
   *     they had. Correct for anything the user triggered mid-session.
   *   "reset" — clears the undo stack and treats the result as the new
   *     baseline. Correct when the host is swapping which document is open,
   *     where undoing into the previous document's content would be wrong.
   */
  const setJson = (
    data: any,
    opts: { history?: "undoable" | "reset" } = {},
  ) => {
    const snapshot = normalizeTemplateSnapshot(data);
    const asVersionShape = {
      canvas: snapshot.canvas,
      content: { rows: snapshot.rows },
    };

    if (opts.history === "reset") {
      // initialize() rather than an undo step: a new baseline, nothing to
      // undo back into. nextTick because loadVersionSnapshot defers its own
      // registry rebuild to one, and initializing history against the state
      // as it exists before that rebuild would snapshot a half-applied tree.
      loadVersionSnapshot(asVersionShape, "set-json");
      nextTick(() => {
        historyManager.initialize(getCurrentStateFn());
        _lastSavedVersion = historyManager.version.value;
      });
  ...
  /**
   * The currently selected node, or null. `type` is the block type
   * ("paragraph", "image", ...) so a host can branch on it without needing
   * to resolve the id against the tree itself.
   */
  const getSelection = (): { id: string; type: string | null } | null => {
    const id = selectedId.value;
    if (!id) return null;
    // componentType, not type. The tree stores the STRUCTURAL kind in `type`
    // — every block node carries type: "component" — and the block kind
    // ("paragraph", "image", ...) in `componentType`. Returning `type` here
    // would hand every caller the constant string "component", which is
    // useless for the branching this field exists to support.
    const node = operations.findComponent(id) as {
      type?: string;
      componentType?: string;
    } | null;
    return { id, type: node?.componentType ?? node?.type ?? null };
  };

  /**
   * Selects a node by id, or clears selection with null.
   *
   * Deliberately does NOT push history — selection is view state, not
   * document state, and an undo stack full of selection changes would bury
   * the edits the user actually wants to undo.
   */
  const setSelection = (id: string | null) => {
    if (id !== null && !operations.findComponent(id)) return false;
    selectedId.value = id;
    return true;
  };

  /**
   * Subscribes to canvas changes. Returns an unsubscribe function.
   *
   * Fires on history version changes rather than a deep watcher on rows:
   * a deep watch over the whole tree would run on every keystroke inside a
   * text block and cost real time on large documents, whereas the history
   * version increments exactly once per committed change — which is the
   * granularity a host actually wants ("something changed, re-evaluate"),
   * not every intermediate keystroke.
   */
  const onChange = (cb: () => void): (() => void) =>
    watch(
      () => historyManager.version.value,
      () => cb(),
    );
