 ...
 */
export const isVisibilityActive = (vis: VisibilityLike | undefined | null): boolean => {
 ...
 */
export const visibilityRuleCount = (vis: VisibilityLike | undefined | null): number => {
  if (!vis) return 0;
  const flatCount  = vis.rules?.length ?? 0;
  const groupCount = (vis.groups ?? []).reduce((sum, g) => sum + (g.rules?.length ?? 0), 0);
