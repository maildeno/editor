...
function renderReactEmail(props: any, ctx: BlockRenderContext): string {
  ...
  const { parseCssString, normalizeFontFamily, styleObj, normalizeInlineStylesToReact } = react;

  const backgroundCss = resolveBgCss(props.backgroundGradient, props.backgroundColor);
  const listClasses = getResponsiveClasses(props.desktopHide, props.mobileHide);

  const listTag = /^\s*<ol/i.test(props.content ?? "") ? "ol" : "ul";
  const liMatches = [...(props.content ?? "").matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)];

  const liItemsJsx = liMatches
    .map((m) => {
      const inner = normalizeInlineStylesToReact(m[1] ?? "").replace(/`/g, "\\`");
