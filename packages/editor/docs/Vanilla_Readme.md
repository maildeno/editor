# @maildeno/editor — Vanilla JS

A drag-and-drop email template editor for plain HTML and JavaScript — no
framework required. This is also the reference for adapting to any framework
not covered by the other guides (Svelte, Angular, Solid, Alpine): they all sit
on the same `init()` contract.
...
No `vue`, no `tailwindcss`, no icon font, no CSS import.
...
<script type="module">
  ...
  const handle = await init({ container: "#editor" });
</script>
```

`container` takes a CSS selector or an element.
...
<script type="module">
  import { init } from "@maildeno/editor/init";
  import { myAdapter } from "./editor-adapter.js";
  ...
  handle.on("save", ({ templateId }) => {
    history.replaceState(null, "", `?id=${templateId}`);
  });
...
Once published, the package works straight from a CDN:

```html
<div id="editor" style="height: 100vh"></div>

<script type="module">
  import { init } from "https://esm.sh/@maildeno/editor/init";
...
The pattern is the same everywhere: create a container, `init()` on mount,
`destroy()` on unmount.
...
let container;
let handle;
onMount(async () => { handle = await init({ container }); });
...
async ngAfterViewInit() {
  this.handle = await init({ container: this.container.nativeElement });
}
ngOnDestroy() { this.handle?.destroy(); }
```

Because it's a native custom element, no framework-specific wrapper is needed
in either case.

## Light DOM mode

The editor renders in a shadow root by default. Opt out with
`shadowDom: false` if you need light-DOM access — the trade-off is that your
page's CSS can then reach the editor and vice versa.
